...
subject='01';
session=2;
sample=[1,2]; % take the first 2 samples;

force_finger_random=load_random(path,subject,session,'force');
emg_finger_random=load_random(path,subject,session,'preprocess');

force_finger_random=force_finger_random(1,sample);
emg_finger_random=emg_finger_random(1,sample);
...
for v=1:size(emg_finger_random,2)
    v
    emg=emg_finger_random{1,v};
